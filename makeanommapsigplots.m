function makeanommapsigplots(anommapsig,ixz,basepath)
...
% Plotting Parameters
timelim  = [datenum(2014,6,1) datenum(year(datetime('now'))+1,1,1)]; %#ok<DATNM> 
...
for k=1:length(vars)
    ...
    for n=ixz(1):ixz(end)
        if any(any(nanmean(anommapsig.(vars{k})(:,:,n),3))) %#ok<NANMEAN> 
            ...
            print(fullfile(varpath,[anommapsig.line(1:2),'_anommapsig_xz_' vars{k} '_' dnstr]),'-depsc')
        end
    end
end
...
for k=1:length(vars)
    ...
    for n=1:length(anommapsig.sigma)
        if any(~isnan(anommapsig.(vars{k})(n,:)))
            ...
            print(fullfile(varpath,[anommapsig.line(1:2) '_anommapsig_xtime_' vars{k} '_' zstr '.eps']),'-depsc')
        end
    end
end
...
for k=1:length(vars)
    ...
    for n=11:10:41
        ...
        print(fullfile(varpath,[anommapsig.line(1:2), '_anommapsig_timez_' vars{k} '_' xstr]),'-depsc');
    end
end
    ...
    for nx=11:10:41
        for nz=1:length(anommapsig.sigma)
            if any(~isnan(anommapsig.(vars{k})(nz,:)))
                ...
                print(fullfile(varpath,[anommapsig.line(1:2) '_anommapsig_time_' vars{k} '_' zstr '_' xstr '.eps']),'-depsc')
            end
        end
    end
end