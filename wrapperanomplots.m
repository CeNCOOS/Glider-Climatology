function wrapperanomplots(lineID,monthback,basepath)
...
load(['map' lineID(1:2) '.mat'],'mapem','maptmem','mapsigem')
load(['map' lineID(1:2) '.mat'],'anommapem','anommaptmem','anommapsigem')

ixz = find(xzrng(1)<=ut2dn(mapem.time) & ut2dn(mapem.time)<=xzrng(2)); % mapem time is not correct need to check what is up
%ixzkeyboard;
% 
makemapplots(mapem,maptmem,ixz,basepath)

makemapsigplots(mapsigem,ixz,basepath)

makeanommapplots(anommapem,anommaptmem,ixz,basepath)

makeanommapsigplots(anommapsigem,ixz,basepath)
