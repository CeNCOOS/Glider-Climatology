function makemapplots_woa(map,maptm,ixz,basepath)
...
for k=1:length(vars)
    ...
    for n=ixz(1):ixz(end)
        if any(any(nanmean(map.(vars{k})(:,:,n),3))) %#ok<NANMEAN> 
            ...
            print(fullfile(varpath,[map.line(1:2),'_woa_map_xz_' vars{k} '_' dnstr]),'-depsc')
            print(fullfile(varpath,[map.line(1:2),'_woa_map_xz_' vars{k} '_' dnstr]),'-djpeg')
        end
    end
end
...
for k=1:length(vars)
    ...
    for n=1:length(maptm.depth)
        ...
        print(fullfile(varpath,[maptm.line(1:2) '_woa_map_xtime_' vars{k} '_' zstr]),'-depsc')
        print(fullfile(varpath,[maptm.line(1:2) '_woa_map_xtime_' vars{k} '_' zstr]),'-djpeg')
    end
end
...
for k=1:length(vars)
    ...
    for n=11:10:41
        ...
        print(fullfile(varpath,[maptm.line(1:2) '_woa_map_timez_' vars{k} '_' xstr]),'-depsc');
        print(fullfile(varpath,[maptm.line(1:2) '_woa_map_timez_' vars{k} '_' xstr]),'-djpeg');
    end
end