function makemapsigplots_woa(mapsig,ixz,basepath)
...
for k=1:length(vars)
    ...
    for n=ixz(1):ixz(end)
        if any(any(nanmean(mapsig.(vars{k})(:,:,n),3))) %#ok<NANMEAN> 
            pxz(mapsig,vars{k},[],n,[],[],[]);
            dnstr = char(ut2ds(mapsig.time(n),'yyyy-mm-dd'));
            print(fullfile(varpath,[mapsig.line(1:2) '_woa_mapsig_xz_' vars{k} '_' dnstr]),'-depsc');
        end
    end
end
...
for k=1:length(vars)
    ...
    for n=1:length(mapsig.sigma)
        if any(~isnan(mapsig.(vars{k})(n,:)))
            ...
            print(fullfile(varpath,[mapsig.line(1:2) '_woa_mapsig_xtime_' vars{k} '_' zstr '.eps']),'-depsc')
        end
    end
end
...
for k=1:length(vars)
    ...
    for n=11:10:41
        ...
        print(fullfile(varpath,[mapsig.line(1:2) '_woa_mapsig_timez_' vars{k} '_' xstr]),'-depsc');
    end
end