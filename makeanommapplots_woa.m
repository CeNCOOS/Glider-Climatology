function makeanommapplots_woa(anommap,anommaptm,ixz,basepath)
...
for k=1:length(vars)
    ...
    for n=ixz(1):ixz(end)
        if any(any(nanmean(anommap.(vars{k})(:,:,n),3))) %#ok<NANMEAN> 
            ...
            print(fullfile(varpath,[anommap.line(1:2),'_woa_anommap_xz_' vars{k} '_' dnstr]),'-depsc')
        end
    end
end
...
for k=1:length(vars)
    ...
    for n=1:length(anommaptm.depth)
        ...
        print(fullfile(varpath,[anommaptm.line(1:2) '_woa_anommap_xtime_' vars{k} '_' zstr]),'-depsc')
    end
end
...
for k=1:length(vars)
    ...
    for n=11:10:41
        ...
        print(fullfile(varpath,[anommaptm.line(1:2) '_woa_anommap_timez_' vars{k} '_' xstr]),'-depsc');
    end
end
...
for k=1:length(vars)
    ...
    for nx = 11:10:41
        for nz=1:length(anommaptm.depth)
            ...
            print(fullfile(varpath,[anommaptm.line(1:2) '_woa_anommap_time_' vars{k} '_' zstr '_' xstr]),'-depsc');
        end
    end
end