function ptimeONIAllLinesEnd(mapstruct,var1str,iz,nx,oni)
...
% Plot timeseries for the input parameters:
    ...
    % 'nx':         index from end to average over to make plot
...
xstr = ['Offshore ' num2str(x(end)-x(end-nx)) ' km'];
if isscalar(iz)
...
% Plot
var1 = squeeze(nanmean(nanmean(mapstruct{1}.(var1str)(iz,nx:end,:),1),2)); %#ok<NANMEAN> 
...
hold(ax(2),'on');
for iiline=2:3
    var1 = squeeze(nanmean(nanmean(mapstruct{iiline}.(var1str)(iz,nx:end,:),1),2)); %#ok<NANMEAN> 
...
xmax = datenum(2019,1,1); %#ok<DATNM> 
xtick = datenum(2007:2019,1,1); %#ok<DATNM> 
